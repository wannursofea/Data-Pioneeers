...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      home: MainPage(user: StudentData(name: "Guest", email: "guest@example.com", uid: '', password: '')),
      routes: {
        '/profile': (context) => ProfilePage(),
        '/edit_profile': (context) => EditProfilePage(),
        '/post_question': (context) => PostQuestionPage(), // Add the route for posting questions
        '/exercise': (context) => ExercisePage(), // Add the route for ExercisePage
        '/questions': (context) => QuestionList(),
        '/textbooks': (context) => TextbookPage(), // Add the route for QuestionList
      },
    );
  }
}
...
class MainPage extends StatelessWidget {
  ...
  final List<String> recentHistory = ["Sejarah Textbook", "Math Notes", "Science Homework"];
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text("Hello, Welcome 👋", style: TextStyle(fontSize: 24)),
                        Text(user.name, style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)), // Use user's name
                      ],
                    ),
                  ),
                  ...
                  CircleAvatar(
                    radius: 30,
                    backgroundImage: AssetImage('assets/user_profile.png'), // Display profile picture
                  ),
                ],
              ),
              SizedBox(height: 20),
              Text("Empowering Your Learning Journey, No Matter the Challenges!", style: TextStyle(fontSize: 22, color: Colors.blueAccent)),
              SizedBox(height: 20),
              ...
              Text("Continue where you left off", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
              SizedBox(height: 10),
              ListView.builder(
                shrinkWrap: true, // Ensure it doesn't take more space than needed
              ...
              // Exercise Section
              Text("Exercise Section", style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
              SizedBox(height: 10),
              Card(
                child: ListTile(
